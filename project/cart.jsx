...
const CheckoutSheet = ({ open, onClose, cart, openToast }) => {
  const [customer, setCustomer] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('crumbs-customer') || 'null');
      return saved || { name: '', phone: '', pickup: '', note: '' };
    } catch { return { name: '', phone: '', pickup: '', note: '' }; }
  });
  const [step, setStep] = useState('form'); // form | summary
  const [copied, setCopied] = useState(false);
  const text = useMemo(() => buildOrderText(cart, customer), [cart, customer]);

  useEffect(() => { if (!open) { setStep('form'); setCopied(false); } }, [open]);

  const valid = customer.name.trim() && customer.phone.trim();

  const goSummary = () => {
    if (!valid) return;
    // remember user details for next time (note left as-is, fresh per order)
    ...
    <Sheet open={open} onClose={onClose}>
      {step === 'form' ? (
        <div style={{ padding: '4px 22px 22px' }}>
          ...
          <div className="serif-italic" style={{ fontSize: 14, color: 'var(--ink-3)', marginBottom: 22 }}>
            {customer.name ? `welcome back, ${customer.name.split(' ')[0]}.` : 'tell us where this box is going.'}
          </div>
